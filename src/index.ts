...
const NAME_RE = /^[a-z0-9@][a-z0-9._@/-]*-scorer$/;

function validateName(value: string | undefined): string | undefined {
  if (!value) return 'Project name is required';
  if (!value.endsWith('-scorer')) return 'Project name must end with "-scorer"';
    ...
    .action(async (projectNameArg?: string) => {
      ...
      try {
        await downloadTemplate(
          'github:orienteering-nz/create-scorer/templates/default',
          {
            dir: projectDir,
          },
        );
